...
try:
    from kernels.sm75.moe import expert_counts as _expert_counts
...
class BiBoMoELayer(nn.Module):
    ...
    def _balance_step(self, top_k_indices, num_tokens):
        ...
        if _expert_counts is not None and _flat.is_cuda:
            # tkf block-local histogram: scatter_add_ serialized ~393k atomics on 64 bins
            current_tpe = _expert_counts(_flat, self.num_routed_experts)
        else:
