...
class BiBoRotaryEmbedding(nn.Module):
    ...
    def forward(self, x, seq_len=None):
        ...
        Returns:
            Tuple of (cos, sin) embeddings with shape (seq_len, head_dim)
        """
...
def apply_rotary_pos_emb(q, k, cos, sin, unsqueeze_dim=1):
    ...
    Args:
        ...
        unsqueeze_dim: Dimension to unsqueeze (default=1 for num_heads dimension)
        
    Returns:
        Tuple of (q_embed, k_embed) with rotary embeddings applied
    """
    # Unsqueeze to add batch and num_heads dimensions: (seq_len, head_dim) -> (1, 1, seq_len, head_dim)
...
class BiBoAttention(nn.Module):
    ...
    def __init__(self, config: BiBoConfig, layer_idx: int, use_sliding_window: Optional[bool] = False):
        ...
        if self.use_ssmax:
            # SSMax scale initialization fix:
            # Init so ssmax_scale * log(typical_kv_len) ≈ 1.0 at step 0
            # Without this, scale ≈ log(2048)/sqrt(128) ≈ 0.68 → attention collapses early
            # See docs/ssmax.md for detailed explanation
