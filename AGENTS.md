...
- **(June 24 2026) Grouped-GEMM MoE** (`moe_grouped.py`) — forward ~2–2.5x, fwd+bwd ~2x at 4k–8k tokens vs eager (fp16). **Certify the 16384-tok training shape on T4** — the 4GB RTX 3050 can't measure it (thermal + allocator pressure). Cert harness: `src/kernels/.autoresearch/bench_real.py`.
- **(June 27 2026) MoE default PROMOTED to `patch_moe_auto`** — `bench/models.py` now patches BiBo's MoE with `patch_moe_auto` (was `patch_moe_with_triton`/per-expert). Auto-dispatch: grouped-GEMM path at `n_tokens >= GROUPED_MIN_TOKENS` (4096), per-expert below — the 16384-tok training shape (B16×S1024) hits the grouped path. Trigger was a head-to-head vs `bassrehab/triton-kernels`' fused MoE forward (forward-only, no backward): against our OLD per-expert default their fused forward was ~2× faster; against the grouped path it's a **tie** (RTX 3050 fp16 eager, identical routing+weights: their_ffn/grp_fwd 0.81–1.06×, output match 1.95e-3). The grouped path's matched grouped-GEMM **backward** is also **1.4–3.7× faster than the per-expert default's** fwd+bwd at 1k–4k tok — so promoting both ties their forward AND speeds our backward, no trade-off. ⚠️ Still **T4-uncertified at 16384 tok** (4GB laptop can't run it; local numbers are throttle-noisy and directional only) — the cert in the line above is now a post-promotion verification, not a gate. `patch_moe_with_triton` (per-expert) and `patch_moe_grouped` (grouped-everywhere) remain available for A/B.
