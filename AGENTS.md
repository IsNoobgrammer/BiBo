...
| `routed_scaling_factor` | 1.0 | MiMo/DeepSeek-V3 final routed-weight scale, applied after `norm_topk_prob`. 1.0 = no-op (MiMo-V2.5). **Measured Jul 27 2026** (137M, 500 steps, 30 GLU experts, top_k=4, sum-norm, normuon, seed 42069, bpb_overall): `1.0` 0.95195 / `1.5` 0.95925 / `2.0` 0.96668 / `2.5` 0.97780 — **monotonically harmful above 1**, and +0.0259 at 2.5 clears the ~0.02 single-seed bar, so DeepSeek-style large values are wrong here. `routed_scaling_learnable` (per-layer scalar) reached 0.94705 and settled at mean **0.958** (min 0.887, max 1.093), i.e. **on par with the fixed default of 1.0** — the −0.0049 gap is inside the 0.007 between-seed sigma, so learnability is NOT adopted. Note the grid was one-sided (never sampled <1) and the model moved below 1, so the true optimum may be slightly under 1; untested. |
...
| `partial_rotary_factor` / `swa_partial_rotary_factor` | **0.0 / 1.0** (NoPE global, full RoPE local) | **Dim-wise** partial RoPE: fraction of EACH head's `head_dim` that rotates; the rest is NoPE. Replaces head-wise `rope_nope_ratio` (Jul 1 2026). `rope_dim=even(round(factor·head_dim))`. 1.0 = full RoPE. No KV-alignment constraint (applies to all heads uniformly). |
...
- **(July 8 2026) TRAINING PRECISION: bf16/fp32 ONLY — fp16 REMOVED.** `train.py` `AMP_DTYPE` now resolves from `BIBO_AMP_DTYPE=bf16(default)|fp32` and hard-errors on fp16; `GradScaler` is gated to fp16 (now always disabled); `eval.py`/`compare.py` autocast switched to bf16. Reason (diagnosed on RTX 3050, vocab 32k / 500 steps, full 2×2 ablation): fp16 training NaNs "after a certain point" with NO gradient blow-up. Root cause = **Muon + fp16 forward**: Muon's orthogonalized updates are gradient-INDEPENDENT (magnitude ≈ lr·0.2·√dim), so they don't shrink as the loss plateaus; with no weight bound the 2D/3D weights creep up (max|param| 1.0→2.8 over ~200 steps) until a **MoE PolyGLU expert forward activation exceeds fp16's 65504 → inf → NaN** (instrumented: first finite→inf op = `layers.N.mlp.experts`). fp32/bf16 forward = stable (8-bit exponent range); AdamW = stable (tracks the shrinking grad). NS `ns_dtype` is IRRELEVANT (forward overflows, not NS); wd only DELAYS. Note the relu2 fp32-square guard (moe.py:134 / experts.py:53) is incomplete — it casts r² back to fp16, so gate>256 still stores inf. Follow-ups if fp16 speed is ever needed on Turing: bound weights (Muon `spectral_wd` / wd + LR decay) AND make the expert fp16-safe (clamp gate / upcast down_proj+relu2-store). Supersedes the "T4 is fp16-only" lesson below for TRAINING; kernel *parity/benchmark* checks may still exercise fp16.
