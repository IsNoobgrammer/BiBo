...
def build_optimizers(model, muon_lr=3e-4, adam_lr=3e-4, wd=0.1, momentum=0.95, ns_dtype=torch.bfloat16,
    ...
    # A name matching BOTH predicates would be appended to two param groups and AdamW would raise
    # "some parameters appear in more than one parameter group". Nothing matches both today; assert
    # it rather than rediscover it as a crash three arms from now.
    _dbl = [n for n, _ in other if _is_typed_read(n) and _is_as(n)]
    assert not _dbl, f"param in both typed-read and act-scale groups: {_dbl}"
