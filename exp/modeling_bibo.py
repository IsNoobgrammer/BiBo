...
class BiBoDecoderLayer(nn.Module):
    ...
    def _forward_attention_residuals(
        ...
        if self.use_typed_attn_res:
            # thought_residual is reset to None at the SAME block boundary as prefix_sum (above),
            # so it needs the identical fp32 restart. Without it the typed stream runs bf16 where
            # the canonical stream runs fp32 -- which is exactly the confound attn_res_fp32_stream
            # was added to remove, and it made the AttnRes arms non-comparable to their baseline
            # once already.
            if thought_residual is None:
                thought_residual = (attn_output.to(_stream_dtype) if self.attn_res_fp32_stream
                                    else attn_output)
            else:
                thought_residual = thought_residual + attn_output
